...
from schemas import NoteModel, NoteUpdate, NoteStatusUpdate
...
async def remove_note(note_id: int, db: Session) -> Note:
...
async def update_note(note_id: int, body: NoteUpdate, db: Session) -> Note:
    note = db.query(Note).filter(Note.id == note_id).first()
    if note:
        tags = db.query(Tag).filter(Tag.id.in_(body.tags)).all()
...
async def update_status_note(note_id: int, body: NoteStatusUpdate, db: Session) -> Note:
    note = db.query(Note).filter(Note.id == note_id).first()
    if note:
        note.done = body.done
        db.commit()
